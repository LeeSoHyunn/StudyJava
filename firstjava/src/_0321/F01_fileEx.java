...
public class F01_fileEx {
	...
	public static void main(String[] args) {
		...
		//File 객체는 파일과 디렉터리 작업을 위해 사용하는 객체
		File f = new File("D:/Filetest/a.txt");
		File dir = new File("./");    //현재 작업위치  //상대경로
		//뒤에 /가 붙으면 디렉터리
		...
		//2. isDirectory() : 디렉터리 여부 확인(파일이면 false)
		...
		//상대경로,, 절대경로: 최상위 경로부터 파일의 경로를 다 적는것..
		...
		//mkdir : 해당 경로에 폴더 생성. 단.. 경로상 없는 폴더가 없어야 한다.
		//mkdirs : 존재하지 않는 부모 폴더까지 모두 포함하여 폴더를 생성.
		if(!test.exists()) { //파일 폴더가 존재하지 않는 경우.. 생성
			test.mkdirs();
		}
...
/*		test = new File("D:/Filetest/temp/abc");
	...
	*/
		...
		//renamTo : 파일이나 폴더의 이름 및 경로를 변경합니다
		test = new File("D:/Filetest/temp");

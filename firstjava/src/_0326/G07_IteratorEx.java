...
/*
	스트림 : 여러값들을 가지고 있는 배열이나 콜렉션 등을 for문이 아닌 함수형 인터페이스
	       를 이용해서 반복 처리할 수 있는 기능(자바의 필수 기능은 아니지만 가독성 좋은 코드를 생성하기 위해서 알아둬야함)
	...
	스트림의 처리단계
	1. 생성 : 스트림 인스턴스 생성
	2. 가공 : 필터링과 매핑 등 원하느 결과를 만들어가는 중간 작업
	3. 결과 : 최종적으로 결과를 얻어내는 작업
	
	외부 반복자 vs 내부 반복자
	외부 반복자 : 컬렉션 내에서 요소를 반복문을 통해서 하나씩 읽어 처리하는 방식 (=for문)
...
*/
	public static void main(String[] args) {
		...
		System.out.println("내부 반복자 이용");  //스트림을 통해서

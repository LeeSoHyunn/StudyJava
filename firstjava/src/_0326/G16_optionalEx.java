...
/*
		집계 메서드
		 스트림 API에서 요소들의 최소값, 최대값, 합계, 평균값 갯수 등을 구할 수 있는 메서드를 하고 있음.
		 합계와 객수를 구해주는 메서드 이외 Optional로 시작하는 타입으로 리턴.
*/
	...
	public static void main(String[] args) {
		...
		//요소 값이 있는 경우에만 출력
		if(avg2.isPresent()) { //값이 있으면 출력
		System.out.println("요소들의 평균 : "+avg2.getAsDouble()); //오류 : No value present 값이없음
		}
		...
		System.out.println("sum()을 이용한 면접 합계: "+ areaSum);
		
		areaSum = list.stream().mapToDouble(Shape::area)
				.reduce((a, b) -> a+b).getAsDouble();
		System.out.println("reduce(Operator)를 이용한 면적합계 : "+ areaSum);
		
		areaSum = list.stream().mapToDouble(Shape::area)
				.reduce(0, (a,b) -> a+b);
		System.out.println("reduce(0,Operator)를 이용한 면적합계 : "+areaSum);

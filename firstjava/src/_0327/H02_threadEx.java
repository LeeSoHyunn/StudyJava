...
*/
	public static void main(String[] args) {
		...
		//Runnable 구현 <-implements 한것을 집어넣음
		//펑셔널인터페이스 람다식 표현가능
		...
		//스레드 우선순위
		//스레드가 가질 수 있는 우선순위 1~10 사이의 숫자로 10이 가장 높은 우선순위임
		...
		//직접 스레드의 우선 순위값을 설정하는 것보다 Thread의 static final 필드값을 이용하는 것이 더 좋다
...
class ThreadExtend extends Thread {
	...
	public void run() {
		...
		for(int i =0; i<50; i++) {    //그냥 보이는대로 0-49까지 출력
...
class RunableImpl implements Runnable {
	...
	public void run() {
		...
		for(int i =0; i<50; i++) {   
			System.out.println("RunableImpl: "+i);
		}
	}
}
...
public class BoardController extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/board/list.board")) { //글목록
		...
		}else if(command.equals("/board/write.board")) { //글작성페이지
			...
			String num = request.getParameter("num"); //게시글 번호를 가져가면서 컨트롤로감
			response.sendRedirect("content.board?num="+num); //게시글 번호를 가져와야됨
			 ...
			 * 3.DAO delete()에서는 delete구문으로 삭제
			 * 4. 페이지 이동을 목록으로
			 * 
			 * sql = "delete from board whre num=?"
			 * */
